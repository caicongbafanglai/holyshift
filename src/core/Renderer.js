...
export class Renderer {
  constructor(container, quality = 'auto') {
    ...
    this.instance.setClearColor(0x070914, 1);
    ...
    this.autoStartingRatio = Math.min(window.devicePixelRatio || 1, 0.7);
    const initialRatio =
      quality === 'auto'
        ? this.autoStartingRatio
        : Math.min(window.devicePixelRatio || 1, QUALITY_RATIOS[quality]);
    this.pixelRatio = Math.max(0.7, initialRatio);
  ...
  setQuality(quality) {
    ...
    const requested =
      quality === 'auto'
        ? this.autoStartingRatio
        : Math.min(window.devicePixelRatio || 1, QUALITY_RATIOS[quality]);
    this.setPixelRatio(Math.max(0.7, requested));
    this.slowWindows = 0;
    this.fastWindows = 0;
  }
  ...
  recordFrame(deltaSeconds) {
    ...
    if (this.quality !== 'auto' || now - this.lastEvaluation < 1600 || this.frameTimes.length < 30) {
      return;
    }
    ...
    if (this.slowWindows >= 2 && this.pixelRatio > 0.72) {
